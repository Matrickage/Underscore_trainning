...
/**
 * Publicis Healthware International edits
 * ---------------------------------------
 *
 * 2011.04.01 - Robert Carbajal
 *	- (webkit_draggable) "horizontal" (boolean) constructor option: If true(default), dragging horizontally is enabled
 *	- (webkit_draggable) "vertical" (boolean) constructor option: If true(default), dragging vertically is enabled
 *	- (webkit_draggable) "onDrag" (function) constructor option: Callback function executed while element is being dragged (in touchMove event)
 *	- (webkit_draggable) "element" member method: Function to retrieve a reference to the specified draggable HTML element
 */
...
var webkit_draggable = function(r, ip)
{
	...
	this.prepare = function()
	{
		...
		//set zindex;
		rs.zIndex = rs.zIndex || webkit_tools.getCalculatedStyle(this.root, 'z-index') || '0';
	}
    ...
    this.setPosition = function(position, value)
    {
		var rs = this.root.style;
	...
	this.touchMove = function(event)
	{
		...
		if(p.horizontal) rs.left = p.rx + delX + 'px';
		if(p.vertical) rs.top  = p.ry + delY + 'px';
